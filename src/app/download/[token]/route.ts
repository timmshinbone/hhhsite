...
import { getPurchase, getLeadRecord } from '@/lib/store';
 ...
 *
 * The token link is permanent so a buyer (or freebie subscriber) can come back
 * months later. The presigned URL it mints lives 15 minutes, so the bucket
 * object is never publicly addressable or indexable.
 */
export async function GET(
  ...
  if (purchase) {
    ...
    const files = purchase.entitlementKeys
  ...
  // ─── Free lead magnet ────────────────────────────────────────────────────
  const lead = await getLeadRecord(token);

  if (lead) {
    const url = await presignDownload(lead.fileKey, `${lead.label}.pdf`);
    return NextResponse.redirect(url, 302);
  }
